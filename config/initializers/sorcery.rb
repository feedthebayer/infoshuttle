...
Rails.application.config.sorcery.configure do |config|
  ...
  # Default: `nil`
  #
  config.cookie_domain = "infoshuttle.bayer.ws"
  ...
  if Rails.env.development?
    config.twitter.callback_url = "http://infoshuttle.dev/oauth/callback?provider=twitter"
  else
    config.twitter.callback_url = "http://infoshuttle.bayer.ws/oauth/callback?provider=twitter"
  end
  ...
  config.user_config do |user|
    ...
    # Default: `[:email]`
    #
    user.username_attribute_names = [:username, :name]
    ...
    # Default: `604800`
    #
    user.remember_me_for = 3.15569e7 # 1 year in seconds
